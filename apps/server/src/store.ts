...
const REPLAY_SNAPSHOT_CACHE_LIMIT = 4
...
export class Store {
  ...
  #readReplaySnapshot: StatementSync
  ...
  constructor(location: string) {
    ...
    for (let mask = 0; mask < 4; mask += 1) {
      ...
      // Global search needs only the FTS index. Filtered searches join thread
      // metadata, while project metadata is added only for the visible page.
      const joins = mask === 0 ? '' : 'JOIN threads ON threads.id = session_search.thread_id'
      if ((mask & 1) !== 0) clauses.push('threads.project_path = ?')
      if ((mask & 2) !== 0) clauses.push('threads.provider = ?')
    ...
    this.#readReplaySnapshot = this.#db.prepare(
      `SELECT seq, payload FROM thread_replay_snapshots WHERE thread_id = ? AND seq = ?`,
    )
  ...
  #updateSidebarThread(id: string, update: SidebarThreadUpdate): void {
    ...
    updated[index] = next
    this.#sidebarThreadsCache = updated
  }
  ...
  /** Persist one cross-thread provider window with one WAL transaction. */
  appendBatch(records: ReadonlyArray<{ threadId: string; event: DomainEvent }>): number[] {
    if (records.length === 0) return []
    const at = Date.now()
    return this.#transaction(() =>
      records.map(({ threadId, event }) => this.#appendEvent(threadId, event, at)),
    )
  }
  ...
  /** Read a derived compact replay only when it matches the exact event tail. */
  replaySnapshot(threadId: string, seq: number): ReplayEntry[] | undefined {
    const cached = this.#replaySnapshotCache.get(threadId)
    if (cached) {
      if (cached.seq !== seq) return undefined
      this.#promoteCachedReplaySnapshot(threadId, cached)
      return cached.entries
    }
    const row = sqliteRow<ReplaySnapshotRow>(this.#readReplaySnapshot, threadId, seq)
    if (!row) return undefined
    return this.#cacheReplaySnapshot(threadId, Number(row.seq), row.payload)
  }
  ...
  /** Read a replay snapshot only when it still matches the current event tail. */
  tailReplaySnapshot(threadId: string): ReplayEntry[] | undefined {
    const cached = this.#replaySnapshotCache.get(threadId)
    if (cached) {
      this.#promoteCachedReplaySnapshot(threadId, cached)
      return cached.entries
  ...
  #pruneSearchSnapshots(now: number, reserveSlot: boolean): void {
    ...
    if (this.#searchSnapshots.size <= retainedCount) return
    const active = [...this.#searchSnapshots].sort(
      (left, right) => right[1].expiresAt - left[1].expiresAt,
    )
    for (const [snapshotId] of active.slice(retainedCount)) {
      this.#searchSnapshots.delete(snapshotId)
    }
  }
