...
function writeEventsBatched(location: string, count: number): void {
  const store = new Store(location)
  store.addProject('/benchmark')
  store.addThread({
    id: 'thread-1',
...
describe('streamed event persistence', () => {
  ...
  bench(
    'persists 20,000 in-memory text deltas in shared windows',
    () => writeEventsBatched(':memory:', 20_000),
    IN_MEMORY_OPTIONS,
  )
  ...
  bench(
    'persists 10,000 file-backed text deltas in shared windows',
    () => {
      const location = path.join(benchmarkDirectory, `batch-events-${databaseIndex++}.sqlite`)
      writeEventsBatched(location, 10_000)
    },
    FILE_OPTIONS,
  )
