...
describe('response serializer', () => {
  ...
  it('seeds the cache with JSON already produced by an immutable snapshot', () => {
    const cache = createSerializedResultCache<{ events: unknown[]; running: boolean }>()
    const value = { events: [{ seq: 1 }], running: false }
    const encoded = JSON.stringify(value)
    const seeded = cache(value, encoded)

    expect(seeded.json).toBe(encoded)
    expect(cache(value)).toBe(seeded)
    expect(JSON.parse(serializeSuccessResponse('history', seeded))).toEqual({
      id: 'history',
      result: value,
    })
  })
