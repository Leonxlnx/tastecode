...
describe('events', () => {
  ...
  it('uses only an exact replay snapshot and drops it across history rewrites', () => {
    const snapshot = [{ seq: 1, event: message('cached') }]
    store.append('t1', message('one'))
    store.saveReplaySnapshot('t1', 1, snapshot)

    expect(store.replaySnapshot('t1', 1)).toBe(snapshot)
    expect(store.tailReplaySnapshot('t1')).toBe(snapshot)
    expect(store.replaySnapshot('t1', 2)).toBeUndefined()

    store.addCheckpoint({ threadId: 't1', seq: 1, commit: 'abc', label: 'One' })
    store.append('t1', message('two'))
    expect(store.tailReplaySnapshot('t1')).toBeUndefined()
    expect(store.replaySnapshotBase('t1')).toEqual({ seq: 1, entries: snapshot })
    const token = store.saveRestoreUndo('t1', 1, 'def')
    expect(store.replaySnapshot('t1', 1)).toBeUndefined()
  ...
  it('bounds parsed replay snapshots and retains the most recently used threads', () => {
    ...
    // Reading t1 makes it newer than t2 before the fifth snapshot arrives.
    expect(store.tailReplaySnapshot('t1')).toBe(snapshots.get('t1'))
    store.addThread({
      id: 't5',
      projectPath: '/repo',
      provider: 'codex',
    ...
    const fifthSeq = store.append('t5', message('thread-5'))
    const fifth = [{ seq: fifthSeq, event: message('snapshot-5') }]
    store.saveReplaySnapshot('t5', fifthSeq, fifth)

    expect(store.tailReplaySnapshot('t1')).toBe(snapshots.get('t1'))
    expect(store.tailReplaySnapshot('t5')).toBe(fifth)
    expect(store.tailReplaySnapshot('t2')).toEqual(snapshots.get('t2'))
    expect(store.tailReplaySnapshot('t2')).not.toBe(snapshots.get('t2'))
  })
  ...
  it('indexes user submission ids and keeps restore exact', () => {
    ...
    expect(store.hasItem('t1', 'i-assistant')).toBe(true)
  ...
  it('persists a cross-thread event window with ordered sequence numbers', () => {
    ...
    const sequences = store.appendBatch([
