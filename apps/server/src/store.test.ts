...
describe('usage event projection', () => {
  it('reads usage history from the usage_events projection, not the event log scan', () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), 'harness-usage-projection-'))
    const file = path.join(dir, 'usage.db')
    const seeded = new Store(file)
    try {
      seeded.addProject('/repo')
      seeded.addThread({ id: 'one', projectPath: '/repo', provider: 'codex', title: 'One' })
      const seq = seeded.append('one', usage(100))
      seeded.append('one', message('noise'))
      // The projection keeps its own copy: even when the event log row no
      // longer matches a usage scan, the indexed usage row still reads back.
      const raw = new DatabaseSync(file)
      try {
        raw.prepare(`UPDATE events SET payload = ? WHERE seq = ?`).run('{}', seq)
      } finally {
        raw.close()
      }

      expect(seeded.usageEvents()).toEqual([
        {
          threadId: 'one',
          provider: 'codex',
  ...
  it('rejects corrupt usage projection rows instead of misreading them', () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), 'harness-usage-projection-'))
    const file = path.join(dir, 'usage.db')
    const seeded = new Store(file)
    try {
      ...
      try {
        raw
          .prepare(`UPDATE usage_events SET payload = ? WHERE event_seq = ?`)
          .run(JSON.stringify({ type: 'usage.updated' }), seq)
      } finally {
        raw.close()
      }

      expect(() => seeded.usageEvents()).toThrow()
    } finally {
      seeded.close()
      rmSync(dir, { recursive: true, force: true })
    }
