...
describe('projects.list projector', () => {
  it('omits renderer defaults but keeps non-default sidebar state', () => {
    ...
    const result = project(projects, threads, new Set(), {
      isTurnRunning: () => false,
      inboxStatus: (threadId) => (threadId === 'b' ? 'ready' : 'idle'),
      revision: () => 0,
    })

    expect(result.projects[0]?.sessions[0]).toEqual({
      id: 'a',
      title: 'A',
      provider: 'codex',
      createdAt: 2,
    ...
    const verbose = createProjectListProjector({ includeDefaults: true })(
      projects,
      threads,
      new Set(),
      {
    ...
    expect(verbose.projects[0]?.sessions[0]).toMatchObject({
      pinned: false,
      status: 'idle',
      unread: false,
      lifecycle: { state: 'active', keepActive: false },
