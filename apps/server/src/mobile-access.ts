...
import type { ConnectionAddress, ConnectionsStatus, PairedDevice } from '@harness/contracts'
...
type PairingTicket = { expiresAt: number }

/**
 * An on-demand second listener for native clients.
 *
 * The desktop stays on the loopback control plane. A QR grants one narrowly
 * scoped bootstrap connection, which is exchanged for a random device token;
 * only its one-way digest is persisted. Tailscale and LAN are transports for
 * the same protocol rather than separate account systems.
 */
export class MobileAccess {
  ...
  #tickets = new Map<string, PairingTicket>()
  ...
  async startPairing(): Promise<ConnectionsStatus & { pairingUri: string; expiresAt: number }> {
    ...
    // Only the QR currently shown in Settings should remain claimable. A user
    // generating a replacement reasonably expects the previous image to stop
    // granting bootstrap access immediately rather than five minutes later.
    this.#tickets.clear()
    const ticket = randomBytes(32).toString('base64url')
    const ticketHash = digest(ticket)
    const expiresAt = Date.now() + PAIRING_TTL_MS
    this.#tickets.set(ticketHash, { expiresAt })
  ...
  claim(
    ...
    const ticket = this.#tickets.get(access.ticketHash)
  ...
  revoke(deviceId: string): void {
    ...
    for (const socket of server.clients) {
      ...
      if (access?.kind === 'device' && access.deviceId === deviceId) {
        socket.terminate()
      }
    }
  }
...
function cleanDeviceName(name: string): string {
  ...
  return cleaned.slice(0, 80) || 'Mobile device'
}
