...
const idleLifecycleScheduler = new LifecycleScheduler(
  () => {
    idleLifecycleStore.dueSnoozedThreads(-1)
    idleLifecycleStore.sidebarSettings()
    idleLifecycleStore.inactiveThreads(-1)
  },
...
describe('thread history loading', () => {
  bench(
    'validates a short replay with separate tail and snapshot reads',
    () => {
      const history = shortHistoryStore.replaySnapshot(
        'short-thread',
        shortHistoryStore.lastSeq('short-thread'),
      )
      if (history?.length !== 1) throw new Error('invalid short replay snapshot')
    },
    OPTIONS,
  )
  ...
  bench(
    ...
    () => {
      const history = shortHistoryStore.tailReplaySnapshot('short-thread')
      if (history?.length !== 1) throw new Error('invalid short replay snapshot')
    },
    OPTIONS,
  )
  ...
  bench(
    'loads the compact replay snapshot for 52,000 persisted events',
    () => {
      const history = historyStore.replaySnapshot(
        'history-thread',
        historyStore.lastSeq('history-thread'),
      )
      if (history?.length !== 1_000) throw new Error('invalid replay snapshot')
    },
    OPTIONS,
  )
  ...
  bench(
    ...
    () => {
      const history = historyStore.tailReplaySnapshot('history-thread')
      if (history?.length !== 1_000) throw new Error('invalid replay snapshot')
    },
    OPTIONS,
  )
...
describe('idle lifecycle refresh', () => {
  bench(
    'checks 10,000 active threads when none are due',
    () => {
      const store = fixtures[1].store
      if (store.dueSnoozedThreads(-1).length !== 0) throw new Error('unexpected snoozed thread')
      if (store.inactiveThreads(-1).length !== 0) throw new Error('unexpected inactive thread')
    },
    OPTIONS,
  )

  bench(
    'polls lifecycle indexes 120 times during one idle hour',
    () => {
      const store = fixtures[1].store
      for (let poll = 0; poll < 120; poll += 1) {
        store.dueSnoozedThreads(-1)
        store.sidebarSettings()
        store.inactiveThreads(-1)
      }
    },
    OPTIONS,
  )
