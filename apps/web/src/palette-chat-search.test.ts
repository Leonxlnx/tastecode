...
describe('deferred palette chat search', () => {
  ...
  it('reads the latest project tree from a stable search function', () => {
    let currentProjects: readonly Project[] = [projects[0]!]
    ...
    expect(search(['keyboard'], 100).map(({ id }) => id)).toEqual(['chat-thread-a'])

    currentProjects = [projects[1]!]
    expect(search(['sidebar'], 100).map(({ id }) => id)).toEqual(['chat-thread-b'])
    expect(search(['keyboard'], 100)).toEqual([])
  })
})
