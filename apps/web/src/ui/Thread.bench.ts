...
import { createThreadProjector, findTurns, presentTurns, projectThreadItems } from './turns.js'
...
const timingFrames = [
  updateTurnTiming({}, timingTurnId, { startedAt: 0, completedAt: 1 }),
  updateTurnTiming({}, timingTurnId, { startedAt: 0, completedAt: 2 }),
]
...
describe('long-thread hot paths', () => {
  bench(
    'derives navigation and presentation in two passes for 1,000 items',
    () => {
  ...
  bench(
    'rebuilds all turn metadata for a structural tail update in 10,000 items',
    () => {
      structuralFrame = structuralFrame === 0 ? 1 : 0
      const items = structuralFrames[structuralFrame]!
      findTurns(items)
      presentTurns(items)
    },
    OPTIONS,
  ...
  bench(
    'rebuilds all presentations for one timing update in 10,000 items',
    () => {
      timingFrame = timingFrame === 0 ? 1 : 0
      presentTurns(structuralFrames[0]!, timingFrames[timingFrame])
    },
    OPTIONS,
  )

  bench(
    'reprojects only the changed turn timing in 10,000 items',
    () => {
      timingFrame = timingFrame === 0 ? 1 : 0
