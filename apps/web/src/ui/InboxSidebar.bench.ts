...
const retainedSelection = new Set(['thread-50-50', 'thread-99-99'])
...
describe('many-thread selection retention', () => {
  bench(
    'builds a 10,000-row membership set for two selected threads',
    () => {
      const ids = new Set(orderedEntries.map((entry) => entry.session.id))
      const retained = new Set([...retainedSelection].filter((id) => ids.has(id)))
      if (retained.size !== retainedSelection.size) throw new Error('lost selected thread')
    },
    OPTIONS,
  )

  bench(
    'checks two selected threads through the retained session index',
    () => {
      const retained = retainInboxSelection(projects, retainedSelection, '', '')
      if (retained.size !== retainedSelection.size) throw new Error('lost selected thread')
    },
    OPTIONS,
  )
...
describe('many-thread bulk menu selection', () => {
  bench(
    'scans 10,000 ordered rows for two selected menu entries',
    () => {
      const entries = orderedEntries.filter((entry) => retainedSelection.has(entry.session.id))
      if (entries.length !== retainedSelection.size) throw new Error('lost selected menu entry')
    },
    OPTIONS,
  )

  bench(
    'loads two selected menu entries through the retained session index',
    () => {
      const entries = resolveInboxSelection(projects, retainedSelection, '', '')
      if (entries.length !== retainedSelection.size) throw new Error('lost selected menu entry')
    },
    OPTIONS,
  )
