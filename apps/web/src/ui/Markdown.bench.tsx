...
const PUNCTUATED_PLAIN_PARAGRAPHS = Array.from(
  { length: 20 },
  (_, index) =>
    `Issue #${index}: keep user_profile_${index} in C:\\workspace\\project-${index}. ` +
    `The result [${index}] is plain prose, not rich formatting.`,
)
...
describe('completed punctuation-heavy prose', () => {
  bench(
    'renders 20 plain replies with paths and identifiers through the full Markdown engine',
    () => {
      const view = render(
        <>
          {PUNCTUATED_PLAIN_PARAGRAPHS.map((text) => (
            <CompletedMarkdown key={text} text={text} />
          ))}
        </>,
      )
  ...
  bench(
    'classifies and renders 20 plain replies with paths and identifiers',
    () => {
      const view = render(
        <>
          {PUNCTUATED_PLAIN_PARAGRAPHS.map((text) => (
