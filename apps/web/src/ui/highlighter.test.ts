...
const workerRequests: HighlighterRequest[] = []
...
describe('syntax highlighter loading', () => {
  it('keeps code plain while its worker loads, then updates every waiting block', async () => {
    ...
    expect(workerRequests.map((request) => request.type)).toEqual(['warm', 'highlight'])
    expect(workerRequests[0]).toMatchObject({ type: 'warm', languages: ['typescript'] })
  })
})
