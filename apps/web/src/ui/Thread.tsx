...
function activityItemsForRender(
  ...
  for (let index = group.firstIndex; index <= group.lastIndex; index += 1) {
    ...
    if (item && isStackedActivity(item)) activity.push(item)
  }
  return activity
}
...
function lastActivityItemForRender({
  ...
  for (let index = group.lastIndex; index >= group.firstIndex; index -= 1) {
    const item = threadItemAt(items, liveItems, index)
    if (item && isStackedActivity(item)) return item
  }
  return undefined
}
...
function ActivityStack({
  ...
  const disclosure = useDisclosure()
  const current = live ? lastActivityItemForRender(activity) : activity.group.items.at(-1)
  const label =
    live && current ? liveActivityLabel(current) : activityStackLabel(activity.group.items)
  const summaryItem = live ? current : activity.group.items[0]
