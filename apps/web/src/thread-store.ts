...
export function reduce(state: ThreadState, event: DomainEvent): ThreadState {
  switch (event.type) {
    case 'turn.started': {
      ...
      const turnTiming = updateTurnTiming(current.turnTiming, event.turn.id, {
        ...current.turnTiming[event.turn.id],
        startedAt,
      })
      return {
        ...current,
    ...
    case 'turn.completed': {
      ...
      const turnTiming =
        event.completedAt === undefined
          ? current.turnTiming
          : updateTurnTiming(current.turnTiming, event.turnId, {
              ...timing,
...
class ReplayItems {
  ...
  appendDelta(itemId: string, textDelta: string, activeTurnId: string | undefined): void {
...
export function reduceEventLog(
  ...
  let deltaText = ''
  let deltaChunksByItem: Map<string, string[]> | undefined
  ...
  const flushDeltas = () => {
    ...
    if (deltaItemId !== undefined) {
      items.appendDelta(deltaItemId, deltaText, activeTurnId)
      deltaItemId = undefined
      deltaText = ''
      return
    }
    for (const [itemId, chunks] of deltaChunksByItem!) {
      items.appendDelta(itemId, chunks.length === 1 ? chunks[0]! : chunks.join(''), activeTurnId)
    }
  ...
  const mutableTurnTiming = () => {
    if (!replayTurnTiming) {
      replayTurnTiming = materializeTurnTiming(next.turnTiming)
  ...
  for (const entry of entries) {
    ...
    if (event.type === 'item.delta') {
      ...
      if (deltaChunksByItem) {
        const chunks = deltaChunksByItem.get(event.itemId)
        if (chunks) chunks.push(event.textDelta)
        else deltaChunksByItem.set(event.itemId, [event.textDelta])
      } else if (deltaItemId === undefined) {
        deltaItemId = event.itemId
        deltaText = event.textDelta
      } else if (event.itemId === deltaItemId) {
        deltaText += event.textDelta
      } else {
        deltaChunksByItem = new Map([
          [deltaItemId, [deltaText]],
          [event.itemId, [event.textDelta]],
        ])
        deltaItemId = undefined
        deltaText = ''
      }
