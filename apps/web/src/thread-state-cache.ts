...
import { releaseThreadItemIndex } from './thread-store.js'
...
const MAX_BACKGROUND_RUNNING_STATES = 8
const MAX_BACKGROUND_RUNNING_ITEMS = 512
const MAX_BACKGROUND_RUNNING_CHARACTERS = 512 * 1024
...
function runningTranscriptUsage(state: ThreadState): RunningTranscriptUsage {
  ...
  let characters = threadMetadataCharacters(state)
...
function threadMetadataCharacters(state: ThreadState): number {
  let characters = 0
  characters += state.activeTurn?.id.length ?? 0
  for (const turnId of Object.keys(state.turnTiming)) characters += turnId.length
...
function compactRunningTranscript(state: ThreadState): ThreadState {
  ...
  delete compacted.usage
  delete compacted.diff
  delete compacted.diffTurnId
  return compacted
}
