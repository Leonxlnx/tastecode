...
Acquire every approved brand typeface as an actual local font file with its license, including when Page omitted a font asset ID. Use only available weights. Verify the official Google Fonts repository or the family's official source; never mark Arial or a system fallback as the acquired custom family. Record one font manifest entry per family and the actual consuming sections.
