...
5. After each batch, reconcile the answers into the brief and reassess what remains. Stop asking as soon as the brief is sufficient. When an open-ended final check would add value, use "Anything else you'd like us to know?" with "No, that's everything" as the recommended choice and a custom answer available; do not add it merely to satisfy a fixed question count. If an answer is genuinely ambiguous, include only the targeted clarification needed in the next batch.
